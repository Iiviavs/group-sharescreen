...
// Where a desktop-app download was started from. Three surfaces offer it and
...
// — so the single `download_app_clicked` event carries this to tell them
// apart in Umami rather than being one undifferentiated count.
//
// A union rather than a loose string, and required at every call site, so a
// fourth surface added later cannot quietly land untracked or invent a
// spelling that splits the metric in two.
export type DownloadSource = "home" | "room-banner" | "install-prompt";
